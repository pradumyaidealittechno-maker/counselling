...
import { Calendar, Clock, Video, CheckCircle, AlertCircle, Plus, MoreVertical, FileText } from 'lucide-react';
...
export default function Sessions() {
    ...
    const upcomingSessions = MOCK_SESSIONS.filter(s => new Date(s.date) >= new Date(new Date().setHours(0, 0, 0, 0))).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
    const pastSessions = MOCK_SESSIONS.filter(s => new Date(s.date) < new Date(new Date().setHours(0, 0, 0, 0))).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            case 'Scheduled': return { bg: 'var(--blue-50)', text: 'var(--blue-700)', icon: Calendar };
            case 'Completed': return { bg: 'var(--success-50)', text: 'var(--success-700)', icon: CheckCircle };
            case 'Cancelled': return { bg: 'var(--error-50)', text: 'var(--error-700)', icon: AlertCircle };
            default: return { bg: 'var(--gray-100)', text: 'var(--gray-700)', icon: Clock };
        }
    };
                        ...
                        return (
                            ...
                            >
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                            ...
                                            {session.meetingLink && <Video size={14} className="text-blue-500" />}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                                    ...
                                    {activeTab === 'past' && session.summary && (
